...
abstract class VideoThumbnailPlatform extends PlatformInterface {
  ...
  /// Batch variant for a thumbnail strip. Opens the video once and extracts
  /// every requested timestamp in a single platform call, so an entire trim
  /// bar (dozens of frames) costs one file-open + N seeks instead of N
  /// separate open/parse/decode cycles. Results align 1:1 with [timeMs];
  /// a failed timestamp maps to `null`.
  Future<List<Uint8List?>> thumbnailDataBatch({
    required String video,
    ...
    throw UnimplementedError('thumbnailDataBatch() has not been implemented.');
  }
}
