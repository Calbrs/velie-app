...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    ...
    } else if ([@"dataBatch" isEqualToString:call.method]) {

        NSArray<NSNumber *> *times = _args[@"timeMs"];

        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:url options:[headers isEqual:[NSNull null]]
                                                                    ? nil : @{
                             @"AVURLAssetHTTPHeaderFieldsKey": headers}];
            // reuse ONE generator + asset for every timestamp in the batch
            ...
            // allow keyframe tolerance so the batch runs at keyframe speed
            imgGenerator.requestedTimeToleranceBefore = CMTimeMake(200, 1000);
            imgGenerator.requestedTimeToleranceAfter = CMTimeMake(200, 1000);

            NSMutableArray *out = [NSMutableArray arrayWithCapacity:times.count];
            for (NSNumber *t in times) {
                NSData *data = nil;
                NSError *error = nil;
                CGImageRef cgImage = [imgGenerator copyCGImageAtTime:CMTimeMake([t intValue], 1000) actualTime:nil error:&error];
                if (cgImage != NULL) {
                    UIImage *img = [UIImage imageWithCGImage:cgImage];
                    CGImageRelease(cgImage);
                    if (format == 0) {
                        data = UIImageJPEGRepresentation(img, quality * 0.01f);
                    } else if (format == 1) {
                        data = UIImagePNGRepresentation(img);
                    } else {
                        data = [NSData data]; // webp unsupported here; keep slot null
                    }
                }
                if (data) {
                    [out addObject:data];
